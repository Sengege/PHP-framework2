  ...
  $menu = '<nav class="navbar navbar-default navbar-static-top">
      <div class="container">
        ...
        <div id="navbar" class="navbar-collapse collapse">
		  ...
		  <li role="presentation" ><a href="/dashboard_cn.php" ><i class="fa fa-dashboard"></i> 主页</a></li>
...
{
  // Start HTML
  startHTML("没有发现小组",false);?>
  
  <?php
  echo $menu;
  echo '<div class="section white"><div class="container"><h2>对不起，这个小组不存在</h2></div></div>';
  footerHTML();
  return;
}
...
else if($groupData['type'] != 'private' && !isGroupMember($groupID))
{
	...
	<div id="groupDescriptionContainer" class="panel panel-default " data-group="<?php echo $groupID; ?>">
            ...
            <div class="panel-heading"><h2><?php echo $groupData['groupName']; ?></h2></div>
				<ul class="list-group">
					<li class="list-group-item"><p><strong>描述</strong></p><p><?php echo $groupData['groupDescription']; ?></p></li>
					<li class="list-group-item"><p><strong>模块详细信息</strong></p><p><?php echo $groupData['module_name']; ?></p><p><?php echo $groupData['module_code']; ?></p></li>
					<li class="list-group-item">
                    <button class="btn btn-danger" id="joinGroup">加入小组</button>                         
                    </li>
				</ul>
	...
	$( document ).ready(function() {
		$("#joinGroup").click(function(e) {
			...
			$(this).html('<i class="fa fa-spinner fa-pulse"></i> 请等待');
			...
			$.get( "/scripts/groups/join/"+groupID, function( data ) {
				  ...
				  {
					bootbox.alert("你已成功加入这个小组!", function() {  location.reload();});  
				  }
				  else
				  {
					bootbox.alert("出错了");
  ...
  <div id="groupDescriptionContainer" class="panel panel-default input-profile" data-group="<?php echo $groupID; /* Essential for edit group to work */?>">
    ...
    <?php if(isGroupAdmin($groupID)) { ?>
          <div class="pull-right" style="padding:3px">
            <button type="button" class="btn btn-default btn-sm" id="editGroup" ><i class="fa fa-edit fa-lg"></i> 编辑</button>
            <button type="button" class="btn btn-default btn-sm" id="saveGroup" style="display:none" ><i class="fa fa-save"></i> 保存</button>
            <button type="button" class="btn btn-default btn-sm" id="cancelSaveGroup" style="display:none" ><i class="fa fa-close"></i> 取消</button>
          </div>
          <?php } ?>
        ...
        <ul class="list-group">
            <li class="list-group-item"><p><strong>描述</strong></p><p><div id="groupDescription" ><?php echo $groupData['groupDescription']; ?></div></p></li>
            <li class="list-group-item"><p><strong>模块详细信息</strong></p><p><?php echo $groupData['module_name']; ?></p><p><?php echo $groupData['module_code']; ?></p></li>
            <li class="list-group-item"><p><strong>成员</strong></p>
             ...
             <li class="list-group-item">
              <?php if(isGroupMember($groupID) && !(isGroupAdmin($groupID)) && !(isDefunct($groupID))) { ?>
                    <button id="leaveGroup" class="btn btn-sm btn-danger">离开小组</button>
                <?php } ?>
                <?php if(isGroupAdmin($groupID) && !(isDefunct($groupID))) { ?>
                       <button id="closeGroup" class="btn btn-sm btn-danger">关闭小组</button>               
                       <?php } ?>
                <?php if(isGroupAdmin($groupID) && !(isDefunct($groupID))) { ?>
                    <button type="button" class="btn btn-default btn-md" data-toggle="modal" data-target="#addMeetingModal">添加会议</button>
                <?php } ?>
                <?php if(isGroupAdmin($groupID) && !(isDefunct($groupID))) { ?>
                    <button id="editUser" class="btn btn-default btn-md" >编辑成员</button>
                <?php } ?> 
                  <?php if(isGroupAdmin($groupID) && isDefunct($groupID)) { ?>
                    <button id="openGroup" class="btn btn-sm btn-danger">恢复小组</button>                  
                <?php } ?>
            </li>
            <li id="slide" style="display:none" class="list-group-item">
              ...
              <br>
                <button id="closeDiv"type="button" class="btn btn-default" >关闭</button>
                <button id="changeMembers" type="submit" class="btn btn-primary">保存成员</button>
              <br>
            </li>
    ...
    <div class="col-md-7">
     <div id="meetingsContainer" data-group="<?php echo $groupID; /* Essential for meetings to work */?>">
        <div class="panel panel-default">
          ...
          <div class="panel-heading"><i class="fa fa-calendar fa-lg"></i>会议</div>
          ...
          <audio id="beepSound" src="/img/beep.mp3" preload="auto"></audio>
              ...
              <form id="addMessageForm">
                            <div class="input-group">
                                <input id="btn-input" type="text" class="form-control input-sm" name="message" placeholder="输入你的信息在这...">
...
<!-- Include all compiled plugins (below), or include individual files as needed -->
 ...
 $.get('/scripts/groups/' + <?php echo $groupID; ?> + '/rating', function(rating){
        ...
        $('.list-group li:eq(1)').after('<li class="list-group-item"><p><strong>小组评价</strong></p><p>' + starHTML + '</p></li>');
      });
...
<div class="modal fade" id="addMeetingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  ...
  <input type="hidden" name="groupID" value="<?php echo $groupID; ?>">
      ...
      <div class="modal-body">
                ...
                <label for="meetingName" class="control-label">会议名称</label>
                <input type="text" name="meetingName" class="form-control" placeholder="请输入会议名称" >
                              
              <br>
              <label>会议内容</label><br>   
              
              <textarea id="agenda" name="agenda" class="form-control" placeholder="请输入会议内容" style="resize: none;"></textarea>
              
              <br>
              <label for="meetingDate" class="control-label">会议日期</label>
              <br>
              <input type="text" name="meetingDate" class="form-control meetingDate"  data-date-format="dd-mm-yyyy" placeholder="日期" /> 
              <br>
              ...
              <div class="control-group">
                ...
                <div class="controls form-inline" id="datesFrom">
                  <span>从</span>
                  <input type="text" name="meetingTime" class="form-control time start" placeholder="开始时间" />
                  <span>到</span>
                  <input type="text" class="form-control time end" placeholder="结束时间"/>
                </div>
              </div>
              <br>
              <label>房间预订 (可选)</label>
              <br><button class="btn btn-default" id="loadRooms" type="button">预定房间</button><br><br>
      ...
      <span class="error" id="meetingError"></span>
        ...
        <button type="submit" class="btn btn-primary" id="addMeetingSubmit">保存</button>
      </div>
    </form>
    </div>
  </div>
...
<div class="modal fade" id="editMeetingModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabelEdit" aria-hidden="true">
  ...
  <form id="editMeetingForm" class="form-horizontal" type="post" action="#">
    <input type="hidden" id="editMeetingID" name="meetingID" value="">
      ...
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabelEdit">编辑会议</h4>
      </div>
      <div class="modal-body">
        
                <label for="meetingName" class="control-label">会议名称</label>
                <input type="text" name="meetingName" id="editMeetingName" class="form-control" placeholder="请输入会议名称" value="Meeting" >
                              
              <br>
              <label>会议内容</label><br>   
              
              <textarea id="editAgenda" name="agenda" class="form-control" placeholder="请输入会议内容" style="resize: none;"></textarea>
              
              <br>
              <label for="meetingDate" class="control-label">会议日期</label>
              <br>
              <input type="text" name="meetingDate" id="editMeetingDate"  class="form-control meetingDate"  data-date-format="dd-mm-yyyy" placeholder="日期" /> 
              <br>
              ...
              <div class="control-group">
                ...
                <div class="controls form-inline" id="editDatesFrom">
                  <span>从</span>
                  <input type="text" name="meetingTime" class="form-control time start" placeholder="开始时间" id="editStartTime" />
                  <span>到</span>
                  <input type="text" class="form-control time end" placeholder="结束时间" id="editEndTime"/>
                </div>
                <br>
              ...
              <br><button class="btn btn-default" id="editBookRoom" type="button">预定房间</button><br><br>
      ...
      <span class="error" id="editmeetingError"></span>
        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
        <button type="submit" class="btn btn-primary" id="editMeetingSubmit">保存</button>
      </div>
    </form>
    </div>
  </div>
...
<script>
    ...
    function saveGroup(group,name,description)
    {
      ...
      $.post( "/scripts/groups/edit", json, function( data ) {
              console.log(data);      
              try { var jData = jQuery.parseJSON(data) } catch (e) { bootbox.alert("<p>出错了</p>"); $("#saveGroup").text('保存小组'); $("#saveGroup").prop("disabled", false); }
              var result = jData.result;
              if(result == 'Successful')
              {
                $("#saveGroup").hide();
    ...
    $('#studentOptions').trigger('chosen:updated');
     ...
     //Change animation
      ...
      $.post("/scripts/groups/action/userAdd", jsonObject, function(data){
       try { var jData = jQuery.parseJSON(data) } catch (e) { bootbox.alert("<p>出错了</p>"); }
        console.log(jData);
        var result = jData.result;
        if(result == 'Successful')
        {
          bootbox.alert("成员列表已更新",function(){
...
function removeStudent(groupID, studentID){
     ...
     bootbox.confirm("你确定要离开小组", function(result){
             ...
             $.post('/scripts/groups/action/remove', postData, function(data){
               ...
               window.location = '/dashboard_cn.php';
   ...
   function closeGroup(groupID){
     ...
     bootbox.confirm("你确定要关闭这个小组？其他小组成员将不能够使用它。"
    , function(result){
             var postData = {
 ...
 /* JAVASCRIPT FOR MEETINGS*/
  ...
  $( document ).ready(function() {
    ...
    $("#meetingTable").on('click','.meetingMoreInfo',function(e){
      ...
      var html = '';
      html += '<div class="col-sm-6"><p><strong>会议内容</strong><br>'+meetingData.agenda+'</p><p><strong>会议日期</strong><br>'+meetingData.meetingDay+' '+meetingData.meetingDateLong+'</p><p><strong>会议时间</strong><br>'+meetingData.startTime+' - '+meetingData.endTime+'</p>';
      if(meetingData.meetingFinished) { html += '<p class="error">这个会议已经过期</p>'; }
      if(meetingData.room.roomID !== null) { 
        var remainingSeats = meetingData.room.seat_capacity - meetingData.attendingNumber;
        html += '<p><strong>位置</strong><br>'+meetingData.room.roomName;
      ...
      html += '</div>';
      html += '<div class="col-sm-6"><p><strong>会议发起者</strong><br>'+meetingData.facilitator.firstName+' '+meetingData.facilitator.lastName+'</p>';
      html += '<p><strong>'+meetingData.attendingNumber+' 人参加</strong><p>';
      ...
      buttonsOptions = {}
      if(meetings["isAdmin"]) { buttonsOptions['warning'] = { label: '<i class="fa fa-edit fa-lg"></i> 编辑', className: "btn-default", callback: function() {  editMeeting(meetingIndex);  }} }
      if(meetings["isAdmin"]) { buttonsOptions['delete'] = { label: '<i class="fa fa-edit fa-lg"></i> 删除', className: "btn-default", callback: function() {  bootbox.confirm("你确定你要删除这个会议吗?",function(result) { if (result) { deleteMeeting(meetingIndex); } }); return false; }} }
      if(meetingData.isAttending ) { buttonsOptions['success'] = { label: '<i class="fa fa-close fa-lg"></i> 取消参加', className: "btn-danger", callback: function() { removeAttendMeeting(meetingData.meetingID); }}}
      if((!(meetingData.isAttending) && remainingSeats > 0) && !(meetingData.meetingFinished) || (!(meetingData.isAttending) && meetingData.room.roomID === null && !(meetingData.meetingFinished) )) { buttonsOptions['success'] =  {label: '<i class="fa fa-plus fa-lg"></i> 参加', className: "btn-success", callback: function() { attendMeeting(meetingData.meetingID);}}}
    ...
    function editMeeting(meetingKey)
    {
      ...
      if(meetingData.room.roomID != null)
      {
        ...
        $("#roomIDContainer").html("<b>Room Booked: </b>"+meetingData.room.roomName+" <br><input type='checkbox' name='removeRoom'>移除房间");
        $("#editBookRoom").text("预定新房间");
        $("#roomIDContainer").show();
      }
      else
      {
          $("#roomIDContainer").hide();
          $("#editBookRoom").text("预定房间");
      }
    ...
    function removeAttendMeeting(meetingID)
    {
      $.get("/scripts/groups/meetings/cancelAttend/"+meetingID, function(data){
        ...
        if(result == 'Successful')
        {
          bootbox.alert("你不再参加这个会议",function(){
            getMeetings();
          })
        }
        else
    ...
    function attendMeeting(meetingID)
    {
      $.get("/scripts/groups/meetings/attend/"+meetingID, function(data){
        ...
        if(result == 'Successful')
        {
          bootbox.alert("现在参加这个会议",function(){
            getMeetings();
          })
        }
        else
      ...
      $.get("/scripts/groups/meetings/get/"+meetings["groupID"], function(data) {
        ...
        if(result == 'Successful')
        {
          ...
          $.each(meetings["meetingData"], function( key, value ) {
            ...
            html += '<td><button type="button" class="btn btn-xs btn-warning meetingMoreInfo" data-meeting="'+key+'">详细信息</button></td></tr>';
            console.log(value);
          })
  ...
  //Hide the LSider
   $("slide").hide();
      ...
      $.post("/scripts/groups/meetings/freeRooms",JSON.stringify(checkObject),function(data){
        console.log(data);      
          ...
          if(result == 'Successful')
          {
            ...
            var html = '<option value="">选择一个房间</option>';
    ...
    $("#newMeetingForm").validate({
      ...
      messages: { 
          meetingName: { required: "请提供一个会议名称！"}, 
          agenda: { required: "请提供会议内容！"},
          meetingDate:{ required: "选择日期"},
          meetingTime: { required: "选择时间"}
        },
      ...
      submitHandler: function(form) {
          ...
          { 
              ...
              if(result == 'Successful')
              {
                ...
                getMeetings();
                bootbox.alert("新的会议添加成功");
                $("#addMeetingSubmit").text("保存");
...
/* JAVASCRIPT FOR CHAT FUNCTION*/
        ...
        $("#addMessageForm").validate({
          ...
          messages: { message: { required: "请输入内容！"}},
          submitHandler: function(form) {
              // fill the json array
