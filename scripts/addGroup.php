...
function addGroup() {
	...
	$insertQuery = "INSERT INTO `groups`(`moduleID`, `adminID`, `groupName`, `groupDescription`, `type`, `createdDate`, `active`) VALUES (:moduleID, :adminID, :groupName, :groupDescription, :groupType, :createdDate,'1')";
	...
	if($q->execute())
	{
		...
		{
			$s->bindParam(':adminID', $eachStudent);
			$s->bindParam(':groupID', $groupID);
			$s->bindParam(':dateJoined', $registrationDate);
			//$s->execute();
			if (!$s->execute()) {
		...
		if ($insert_success) {
		  addNotificationInc("Invitation", $groupID, $student->userID, "");
		}
	}
	else
	{
