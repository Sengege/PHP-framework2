<?php
include 'addNotification.inc.php';
...
function getMeetings($groupID)
{
	...
	if(isGroupAdmin($groupID)) { $jsonMessage['isAdmin'] = true;} else { $jsonMessage['isAdmin'] = false; }
	foreach($fetchMeetings AS $meeting)
	{
		...
		$meetingData['startTime24'] = date("H:i", strtotime($meeting['time']));
		$meetingData['endTime24'] = date("H:i", $endTime);
		$meetingData['duration'] = $meeting['duration'];
		$meetingData['room'] = array("roomID" => $meeting['roomID'],"roomName"=>$meeting['room_number'],"seat_capacity" => $meeting['seat_capacity']);
...
function addMeeting() {
	...
	// Ensure new meeting is not in the past
	...
	if($q->execute())
	{
		...
		addNotificationInc('New Meeting', $groupID, $student->userID, $meetingID);
	}
	else
	{
		// Return unsuccessful json message
